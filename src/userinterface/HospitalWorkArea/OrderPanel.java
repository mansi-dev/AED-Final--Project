...
public class OrderPanel extends javax.swing.JPanel {
        ...
        if (bOrg != null) {
            ...
            for (BloodBank bloodBank : bloodBankList) {
                ...
                if (bloodBank.getCity().equals(city) && bloodStockIns != null && bloodStockIns.getQuantity() > 0) {
                    ...
                    row[0] = bloodBank;
                    row[1] = bloodBank.getName();
                    row[2] = bloodBank.getAddress();
                    row[3] = bloodBank.getCity();
                    row[4] = bloodBank.getState();
                    row[5] = bloodBank.getPhoneNum();
                    row[6] = bloodStockIns.getQuantity();
                    model.addRow(row);
                }
            }
        }
    ...
    private void btnOrderBloodSampleActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnOrderBloodSampleActionPerformed
        ...
        Manager mgr = (Manager) account.getUser();
        if (org != null) {
            BloodBank bloodBank = (BloodBank) model.getValueAt(rowIndex, 0);
            OrderBloodSampleWorkRequest obs = new OrderBloodSampleWorkRequest();
            obs.setBloodGroup(bloodGroup);
            obs.setBloodbank(bloodBank);
            obs.setQuantity(Integer.parseInt(txtQuantity.getText()));
            obs.setStatus("Pending");
            obs.setSender(account);
            obs.setName(mgr.getHospital().getName());
            obs.setHospitalID((int)mgr.getHospital().getId());
